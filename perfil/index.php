      ...
      switch ($op) {
        ...
        if ($id%2 == 0) {
          if(verificarEmail($email)){
            $senha = $_POST['senha-atual'] ?? null;
            if(!is_null($senha)){
              $senhaNova = $_POST['senha-nova'] ?? null;
              $reSenhaNova = $_POST['re-senha-nova'] ?? null;

              if(!is_null($senhaNova) and !is_null($reSenhaNova)){
                $nomeUsuario = $_POST['nome'];
                if($senhaNova == $reSenhaNova and !is_null($nomeUsuario))
                {
                //alterar dados conta
                  $update = "UPDATE usuario SET nm_email = '$email', cd_senha = '$senhaNova', nm_usuario = '$nomeUsuario'";
